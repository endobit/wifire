...
func (ep *ExponentialPredictor) Update(probeTemp float64, timestamp time.Time, probeTargetTemp float64, grillTemp float64, grillSetTemp float64) {
	if !ep.initialized {
...
func (ep *ExponentialPredictor) estimateTimeConstant() {
	...
	for tau := 300.0; tau <= 28800.0; tau += 300.0 { // Step by 5 minutes
		error := ep.calculateErrorWithGrillDynamics(tau, startIdx)
		if error < bestError {
			bestError = error
			bestTau = tau
		}
...
func (ep *ExponentialPredictor) GetCurrentState() (float64, float64) {
	if !ep.initialized {
		return 0, 0
	}
